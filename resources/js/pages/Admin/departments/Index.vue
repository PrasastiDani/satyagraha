...
const deleteDepartment = (department) => {
    const confirmed = window.confirm(`Hapus departemen "${department.name}"?`);
...
<template>
    <AdminLayout title="Departemen" description="Kelola departemen yang digunakan dalam modul lowongan kerja.">
        ...
        <div class="overflow-hidden rounded-[1.75rem] border border-gray-100 bg-white shadow-sm">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-gray-100 bg-gray-50/70">
                            <th class="px-6 py-4 text-left text-[9px] font-black tracking-[0.16em] text-gray-400 uppercase">Departemen</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="department in departments.data" :key="department.id" class="transition hover:bg-gray-50/60">
                            <td class="px-6 py-5">
                                <div>
                                    ...
                                    <p v-if="department.description" class="mt-2 line-clamp-2 max-w-md text-xs leading-relaxed text-gray-500">
                            ...
                            <td class="px-6 py-5">
                                <span
                                    ...
                                    :class="department.is_active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'"
                                >
                                    <span class="size-1.5 rounded-full" :class="department.is_active ? 'bg-green-500' : 'bg-gray-400'" />
                            ...
                            <td class="px-6 py-5">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <button
                                        ...
                                        :disabled="department.job_vacancies_count > 0"
                        ...
                        <tr v-if="departments.data.length === 0">
                            <td colspan="5" class="px-6 py-20 text-center">
                                <span class="material-symbols-outlined text-5xl text-gray-200"> account_tree </span>

                                <p class="mt-3 text-sm font-bold text-gray-500">Belum ada departemen</p>

                                <p class="mt-1 text-xs text-gray-400">Tambahkan departemen untuk mulai mengelola lowongan.</p>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="divide-y divide-gray-100 md:hidden">
                <article v-for="department in departments.data" :key="department.id" class="p-5">
                    ...
                    <p v-if="department.description" class="mt-4 line-clamp-3 text-xs leading-relaxed text-gray-500">
                        {{ department.description }}
                    </p>

                    <div class="mt-4 flex items-center justify-between border-t border-gray-100 pt-4">
                        <p class="text-xs text-gray-400">{{ department.job_vacancies_count }} lowongan</p>
                ...
                <div v-if="departments.data.length === 0" class="px-6 py-16 text-center">
                    <span class="material-symbols-outlined text-5xl text-gray-200"> account_tree </span>
            ...
            <!-- Pagination -->
            <div v-if="departments.links.length > 3" class="flex flex-wrap items-center justify-between gap-4 border-t border-gray-100 px-5 py-4">
                <p class="text-xs text-gray-400">
                    Menampilkan {{ departments.from ?? 0 }}–{{ departments.to ?? 0 }} dari {{ departments.total }} data
                </p>
